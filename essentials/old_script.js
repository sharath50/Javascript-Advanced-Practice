...
let variable2 =
  "we can use let to make it private variable inside local perimeter...";

const variable3 =
  "const variables canno't change just like tuples in python -- immutables";
...
var list4 = [{ name: "karthik" }, { name: "rudresh" }, [1, 2, 3, 4, 5, 6]];
...
class persons extends names {
  ...
  info() {
    if (this.work) {
      console.log(
        `name is ${this.name} age is ${this.age} work is ${this.work}`
      );
    } else if (this.grade) {
      console.log(
        `name is ${this.name} age is ${this.age} work is ${this.grade}`
      );
    }
  }
}
...
class solders extends persons {
  ...
  info() {
    console.log(
      `name is ${this.name} age is ${this.age} and grade is ${this.grade}`
    );
  }
}
...
const account = function(
  ...
  if (membership == "Gold" || membership == "Platinum") {
    console.log(
      `hi ${this.fullname}, thank you for opening account in our bank sector at ${this.branch} branch you got ${membership} membership. enjoy...! `
    );
  } else {
    console.log(
      `hi ${this.fullname}, thank you for opening account in our bank you got bank account in our bank. enjoy...! `
    );
  }
};
...
account.prototype.info = function() {
  ...
  return `name is ${this.fname} age is ${
    this.age
  }, and ${prefix} is working as a ${this.work}, salary would be ${
    this.salary
  } and ${prefix} deposited intially ${this.iDeposite.toFixed(2)}`;
};

account.prototype.deposite = function(amount) {
  this.balance += amount;
  return `amount Rs.${amount} has been deposite right now... balance : ${this.balance.toFixed(
    2
  )}`;
};
...
account.prototype.withdraw = function(amount) {
  if (this.balance >= amount) {
    this.balance -= amount;
    return `amount Rs.${amount} has been withdrawn right now...  balance : ${this.balance.toFixed(
      2
    )}`;
  } else {
    return `hi, you are trying to widraw Rs.${amount} which is more than the balance what you have in your account right now... balance : ${this.balance.toFixed(
      2
    )}`;
  }
};

const online_account = function(
  fname,
  lname,
  age,
  gender,
  ...
  if (membership == "Gold" || membership == "Platinum") {
    if (this.online) {
      console.log(
        `hi ${this.fullname}, thank you for opening online account in our bank sector you got ${membership} membership. enjoy...! `
      );
    } else {
      console.log();
    }
...
const client2 = new online_account(
  "rudresh",
  "karthik",
  19,
  "male",
...
const employees = {
  getInfo: function() {
    return `first name : ${this.fname}, and last name : ${
      this.lname
    } \n name : ${this.fname + this.lname} and employee id : ${
      this.empIdNo
    }, date of join : ${this.dateOfJOin}, designation : ${
      this.designation
    }, salary : ${this.salary}`;
  }
};
